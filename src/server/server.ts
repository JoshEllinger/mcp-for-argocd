...
type ServerInfo = {
  ...
  // server's default -- e.g. a Managed Fusion customer instance logged into
  // via `argocd-mcp login <url>`. Only wired on the stdio transport today
  // (see connectStdioTransport); when omitted, tools cannot target another
  // server and a caller-supplied argocdBaseUrl fails outright.
...
// than this server's configured default. Named to match the equivalent
// parameter upstream (main branch, PR #122's tokenRegistry-based override)
// rather than inventing a second, differently-named mechanism for the same
// purpose -- and deliberately not "server", which list_clusters already uses
// for an unrelated cluster-filter field.
...
export class Server extends McpServer {
  ...
  private addJsonOutputTool<Args extends ZodRawShape, T>(
    ...
    this.tool(name, description, mergedSchema, async (...args) => {
      ...
      // per-call override was given. A server with no default session (e.g.
      // the Managed Fusion server, which has no meaningful "default"
      // customer) must still be usable purely via argocdBaseUrl.
